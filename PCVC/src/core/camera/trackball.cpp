...
Trackball::Trackball() : radius_(0.8f), q_(glm::quat(1.0f, 0.0f, 0.0f, 0.0f)) {}

glm::mat4 Trackball::getRot() const {
...
void Trackball::track(float p1x, float p1y, float p2x, float p2y) {
    if (p1x == p2x && p1y == p2y) {
        return;
    }
...
glm::vec3 Trackball::mapToSphere(float x, float y) const {
    float z;
